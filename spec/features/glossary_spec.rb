...
chos,Link to Here
  ...
  scenario "user can not manipulate glossaries or definitions that are not their own" do
    user1 = create_user("bob@bob.com")
    user2 = create_user("joe@joe.com")

    glossary = create_public_glossary(user2)
    create_bulk_definitions_for(glossary)
    ...
    expect(page).to_not have_content "Edit Glossary"
    expect(page).to_not have_content "Import Glossary"
    expect(page).to_not have_content "Download CSV"
    expect(page).to_not have_content "Download PDF"
